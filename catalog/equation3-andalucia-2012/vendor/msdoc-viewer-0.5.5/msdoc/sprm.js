...
function parseBrc80(bytes, offset = 0) {
    ...
    return {
        ...
        borderType,
        color,
        nil: brc === 0xffffffff || borderType === 0xff || lineWidth === 0xff,
    };
}
function parseBrc(bytes, offset = 0) {
    if (offset + 8 > bytes.length)
        return null;
    const cv = u32(bytes, offset) >>> 0;
    const tail = u32(bytes, offset + 4) >>> 0;
    const lineWidth = tail & 0xff;
    const borderType = (tail >> 8) & 0xff;
    const flags = (tail >> 16) & 0xffff;
    const space = flags & 0x1f;
    return {
        kind: 'brc',
        raw: cv,
        rawHigh: tail,
        ...
        nil: tail === 0xffffffff || borderType === 0xff || lineWidth === 0xff,
    };
}
function parseShd(bytes, offset = 0) {
    if (offset + 10 > bytes.length)
        return null;
    const cvFore = u32(bytes, offset) >>> 0;
    const cvBack = u32(bytes, offset + 4) >>> 0;
    const ipat = u16(bytes, offset + 8);
    const auto = ipat === 0 && (((cvFore >>> 24) & 0xff) === 0xff || cvFore === 0) && (((cvBack >>> 24) & 0xff) === 0xff || cvBack === 0);
    const nil = ipat === 0 && (cvFore >>> 0) === 0xffffffff && (cvBack >>> 0) === 0xffffffff;
    return { kind: 'shd', cvFore, cvBack, ipat, auto, nil };
}
function parseShd80(bytes, offset = 0) {
    if (offset + 2 > bytes.length)
        return null;
    const raw = u16(bytes, offset);
    const icoFore = raw & 0x1f;
    const icoBack = (raw >> 5) & 0x1f;
    const ipat = (raw >> 10) & 0x3f;
    return { kind: 'shd80', raw, icoFore, icoBack, color: icoBack, ipat, auto: raw === 0, nil: raw === 0xffff };
}
function parseTableBordersOperand(bytes, modern) {
    if (!bytes.length)
        return null;
    const cb = bytes[0] ?? 0;
    const borderSize = modern ? 8 : 4;
    const offset = 1;
    const parse = modern ? parseBrc : parseBrc80;
    if (cb < borderSize * 6 || bytes.length < offset + borderSize * 6)
        return null;
    return {
        top: parse(bytes, offset) || undefined,
        left: parse(bytes, offset + borderSize) || undefined,
        bottom: parse(bytes, offset + borderSize * 2) || undefined,
        right: parse(bytes, offset + borderSize * 3) || undefined,
        horizontalInside: parse(bytes, offset + borderSize * 4) || undefined,
        verticalInside: parse(bytes, offset + borderSize * 5) || undefined,
    };
}
function parseDefTableShdOperand(bytes, firstIndex = 0, modern = true) {
    if (!bytes.length)
        return [];
    const cb = bytes[0] ?? 0;
    const itemSize = modern ? 10 : 2;
    const count = Math.min(22, Math.floor(Math.min(cb, Math.max(0, bytes.length - 1)) / itemSize));
    const out = [];
    for (let i = 0; i < count; i += 1) {
        const shd = modern ? parseShd(bytes, 1 + i * itemSize) : parseShd80(bytes, 1 + i * itemSize);
        if (shd)
            out[firstIndex + i] = shd;
    }
    return out;
}
...
function parseRangeByteOperand(bytes) {
    if (bytes.length < 3)
        return null;
    return {
...
function parseRangeWordOperand(bytes) {
    if (bytes.length < 4)
        return null;
    return {
        range: parseItcFirstLim(bytes, 0),
...
function parseRangeColumnWidthOperand(bytes) {
    if (bytes.length < 4)
        return null;
    const width = u16(bytes, 2);
    return {
        range: parseItcFirstLim(bytes, 0),
        ftsWidth: 3,
        width,
...
function parseRangeBrcOperand(bytes, modern) {
    if (!bytes.length)
        return null;
    const borderOffset = 4;
    const border = modern ? parseBrc(bytes, borderOffset) : parseBrc80(bytes, borderOffset);
    return {
        cb: bytes[0] ?? 0,
        range: parseItcFirstLim(bytes, 1),
        bordersToApply: bytes[3] ?? 0,
        border: border || {},
        extra: bytes.subarray(borderOffset + (modern ? 8 : 4)),
    };
}
function parseTableShadeOperand(bytes) {
    if (bytes.length < 13)
        return null;
    return {
        cb: bytes[0] ?? 0,
        range: parseItcFirstLim(bytes, 1),
        value: parseShd(bytes, 3),
        extra: bytes.subarray(13),
    };
}
...
function parseTInsertOperand(bytes) {
    ...
    return { cb, range: itc, itcFirst: itc.first, ctc, dxaCol, cells };
}
...
export function decodeSprm(sprm, operandBytes) {
    ...
    switch (sprm) {
        ...
        case SprmCodes.sprmTJc90:
        case SprmCodes.sprmTJc: return setMeta('table', 'alignment', u16(bytes, 0), raw, bytes);
        ...
        case SprmCodes.sprmTFCantSplit90:
        case SprmCodes.sprmTFCantSplit: return setMeta('table', 'cantSplit', boolValue(bytes), raw, bytes);
        case SprmCodes.sprmTTableHeader: return setMeta('table', 'header', boolValue(bytes), raw, bytes);
        case SprmCodes.sprmTTableBorders80: return setMeta('table', 'tableBorders', parseTableBordersOperand(bytes, false), raw, bytes);
        case SprmCodes.sprmTTableBorders: return setMeta('table', 'tableBorders', parseTableBordersOperand(bytes, true), raw, bytes);
        ...
        case SprmCodes.sprmTDefTableShd80: return setMeta('table', 'defTableShading', parseDefTableShdOperand(bytes, 0, false), raw, bytes);
        case SprmCodes.sprmTDefTableShd:
        case SprmCodes.sprmTDefTableShdRaw: return setMeta('table', 'defTableShading', parseDefTableShdOperand(bytes, 0, true), raw, bytes);
        case SprmCodes.sprmTDefTableShd2nd:
        case SprmCodes.sprmTDefTableShdRaw2nd: return setMeta('table', 'defTableShading', parseDefTableShdOperand(bytes, 22, true), raw, bytes);
        case SprmCodes.sprmTDefTableShd3rd:
        case SprmCodes.sprmTDefTableShdRaw3rd: return setMeta('table', 'defTableShading', parseDefTableShdOperand(bytes, 44, true), raw, bytes);
        ...
        case SprmCodes.sprmTDxaCol: return setMeta('table', 'columnWidth', parseRangeColumnWidthOperand(bytes), raw, bytes);
        ...
        case SprmCodes.sprmTTextFlow: return setMeta('table', 'textFlow', parseRangeWordOperand(bytes), raw, bytes);
        ...
        case SprmCodes.sprmTSetShdOdd: return setMeta('table', 'setShadingOdd', parseTableShadeOperand(bytes), raw, bytes);
        case SprmCodes.sprmTSetBrc80: return setMeta('table', 'setBorder', parseRangeBrcOperand(bytes, false), raw, bytes);
        case SprmCodes.sprmTSetBrc: return setMeta('table', 'setBorder', parseRangeBrcOperand(bytes, true), raw, bytes);
        ...
        case SprmCodes.sprmTCellPaddingDefault: return setMeta('table', 'cellPaddingDefault', parseCssaOperand(bytes), raw, bytes);
        case SprmCodes.sprmTCellWidth: return setMeta('table', 'cellWidth', parseRangeWidthOperand(bytes), raw, bytes);
        case SprmCodes.sprmTFitText: return setMeta('table', 'fitText', parseRangeByteOperand(bytes), raw, bytes);
