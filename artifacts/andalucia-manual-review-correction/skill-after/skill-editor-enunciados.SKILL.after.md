...
17. En bancos destinados a una aplicación, auditar la cadena completa `documento original → ejercicio canónico → ejercicio mostrado`. Año y convocatoria no bastan: deben coincidir ejercicio, apartados, cifras, signos, potencias, raíces, fracciones, matrices, sistemas, límites, integrales, condiciones, unidades y recursos visuales.
18. Registrar por ejercicio `apartados originales` y `apartados canónicos/mostrados`. Un apartado ilegible o no convertible se conserva y queda marcado para revisión; nunca se elimina silenciosamente. Cada alternativa numerada y cada apartado permanecen asociados a su ejercicio correcto.
...
21. En la salida para aplicaciones, matrices, determinantes, sistemas, funciones a trozos, fracciones, raíces, límites e integrales deben transportarse como estructuras, nunca como texto aplanado o tokens internos. Auditar que el alumno no pueda ver `matrix{}`, `system{}`, `frac{}`, `sqrt{}` ni delimitadores TeX.
