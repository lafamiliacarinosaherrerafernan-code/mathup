...
19 bis. Cuando el original presenta varios objetos etiquetados en una misma línea —por ejemplo, `A=matriz, B=matriz`— conservarlos en una única fila visual, con cada pareja `etiqueta=matriz` atómica y sin signos iguales superpuestos. Si no caben en una pantalla estrecha, permitir desplazamiento horizontal de la fila completa antes que apilar `A` y `B` de forma ambigua o separar una etiqueta de su matriz.
