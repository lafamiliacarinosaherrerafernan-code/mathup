...
## Fidelidad, segmentación y composición visual transversales
...
En la derivada de un cociente, aplicar la regla directamente sobre la expresión visible:

`f'(x)=frac{u'(x)v(x)−u(x)v'(x)}{[v(x)]²}`.

Sustituir inmediatamente `u`, `u'`, `v` y `v'` dentro de esa fracción y simplificar después. No introducir una cadena artificial de abreviaturas `N=…`, `D=…`, `N'=…`, `D'=…` cuando el numerador y el denominador originales caben de forma legible en la propia regla. Estas letras solo se admiten si la expresión es excepcionalmente larga y la abreviatura mejora realmente la comprensión; en ese caso deben definirse con significado y volver a mostrarse sustituidas en la fórmula completa.
...
Esta política es común a 1.º y 2.º de Bachillerato cuando el curso incluye el estudio de asíntotas, con las prohibiciones o técnicas de límites específicas de cada materia:

- **Vertical `x=a`:** identificar primero que `a` es un punto excluido o frontera del dominio y calcular los dos límites laterales `lim_(x→a⁻)f(x)` y `lim_(x→a⁺)f(x)`. Existe asíntota vertical si al menos uno es `+∞` o `−∞`. No basta con anular un denominador: comprobar que no exista una cancelación que elimine la divergencia.
- **Horizontal `y=L`:** calcular por separado `lim_(x→+∞)f(x)` y `lim_(x→−∞)f(x)`. La asíntota puede existir solo en uno de los dos infinitos; indicarlo expresamente.
- **Oblicua `y=mx+n`:** calcular `m=lim_(x→±∞)frac{f(x)}{x}` y exigir que sea finito y distinto de cero; después calcular `n=lim_(x→±∞)[f(x)−mx]`. Mantener el mismo signo de infinito en ambos límites y declarar en qué extremo se obtiene la recta.

Cerrar el estudio enumerando todas las asíntotas y los extremos en los que actúan. No deducir una oblicua solo mediante división de polinomios sin verificar los límites de `m` y `n`, aunque la división pueda usarse como simplificación auxiliar.
...
En Margarita Salas, `Adj(A)` es la matriz formada directamente por los menores complementarios con sus signos, antes de trasponer. No introducir una fase, tabla o matriz separada denominada «cofactores». Usar obligatoriamente:
...
2. Construir directamente `Adj(A)` como una matriz. En cada posición escribir el signo `(-1)^(i+j)` y el determinante real del menor complementario correspondiente; en orden tres deben verse dentro de la matriz los nueve determinantes `2×2`, no una lista vertical ni solo sus valores.
3. Resolver cada determinante `2×2` y escribir la matriz numérica `Adj(A)`.
4. Trasponer y obtener `Adj(A)^T`.
5. Aplicar `A⁻¹=(1/det(A))·Adj(A)^T` y presentar el resultado final.
6. Comprobar `A·A⁻¹=I` cuando sea razonable.

No mezclar esta convención con la nomenclatura en la que «matriz adjunta» ya incluye la trasposición, ni añadir una etapa independiente llamada matriz de cofactores.
...
Cuando se desarrolle un determinante por Laplace, el alumno debe ver cada término completo `elemento × menor complementario` con su signo:

1. Indicar la fila o columna elegida y el patrón de signos.
2. Escribir la suma algebraica completa; por ejemplo, `a₁₁·det(M₁₁)−a₁₂·det(M₁₂)+a₁₃·det(M₁₃)`.
3. Representar cada `Mᵢⱼ` como un determinante `2×2` real con sus cuatro entradas, no como un nombre que salta directamente al valor.
4. Resolver cada determinante `2×2` mostrando los dos productos y su resta.
5. Sustituir los valores en la expansión y simplificar.

Queda prohibido pasar directamente del determinante inicial a un polinomio o número si se ha elegido el desarrollo por fila o columna.
...
#### Punto genérico de una recta

Cuando una resolución necesite elegir un punto genérico de una recta:

1. Escribir primero la recta completa en forma paramétrica, con sus tres coordenadas alineadas bajo una única llave cuando se trabaje en el espacio.
2. Explicar literalmente: «Sea C un punto genérico de la recta» —adaptando únicamente la letra si el enunciado ya reserva `C` para otro objeto—.
3. Escribir `C=(x(λ),y(λ),z(λ))` sustituyendo las tres expresiones paramétricas.
4. Usar después ese punto en las condiciones de pertenencia, perpendicularidad, distancia o intersección.

No introducir directamente una terna parametrizada sin explicar qué punto representa ni de qué forma de la recta procede.
...
Cuando se use una sustitución, no saltar de la integral original a la integral transformada:

1. Definir la sustitución completa, por ejemplo `u=g(x)`.
2. Calcular y mostrar el diferencial `du=g'(x)dx` y despejar `dx` si es necesario.
3. Transformar todos los factores del integrando; no dejar simultáneamente variables `x` y `u` salvo durante una línea de transición explícita.
4. Si la integral es definida, transformar por separado cada límite mediante la sustitución y escribir los nuevos límites antes de integrar.
5. Mostrar la integral resultante completa, con signo, límites, integrando y diferencial correctamente asociados.
...
La cadena pedagógica debe permanecer completa y no usar `Φ`, `Φ⁻¹`, `invNorm` ni nombres equivalentes como sustituto de la lectura de la tabla. Usar estos modelos, adaptando las desigualdades y los datos concretos:

- Cola izquierda: `P(X≤a)=P(Z≤frac{a−μ}{σ})=P(Z≤z)`; escribir «Buscamos `z` en la tabla…», el valor acumulado leído y el resultado.
- Cola derecha: `P(X≥a)=P(Z≥frac{a−μ}{σ})=1−P(Z≤z)`; escribir el acumulado consultado antes de restar.
- Intervalo: `P(a≤X≤b)=P(frac{a−μ}{σ}≤Z≤frac{b−μ}{σ})=P(Z≤z_b)−P(Z≤z_a)`; mostrar los dos valores de tabla.
- Lectura inversa: transformar primero la probabilidad pedida en un acumulado `P(Z≤z)=p`, buscar `p` en la tabla, obtener `z` y solo entonces resolver `frac{x−μ}{σ}=z`.
...
En toda integral definida, escribir primero la integral completa —signo, límite inferior, límite superior, integrando y diferencial— antes de introducir la primitiva o aplicar Barrow. No pasar directamente de la selección de una rama a `F(b)−F(a)`.

### Composición nativa de integrales definidas y evaluación de Barrow

En la interfaz interactiva, toda integral definida debe componerse mediante el motor matemático en estilo de presentación, equivalente a `\displaystyle \int_{a}^{b}f(x)\,dx`. Los límites deben pertenecer tipográficamente al operador integral: el superior se sitúa de forma natural sobre su parte superior derecha y el inferior bajo su parte inferior derecha. La evaluación de una primitiva debe componerse del mismo modo, equivalente a `\displaystyle \left[F(x)\right]_{a}^{b}`.

- No construir el operador y sus límites mediante columnas HTML, cuadrículas CSS, `position:absolute`, caracteres colocados a mano, saltos de línea ni márgenes específicos por ejercicio.
- No simular Barrow apilando manualmente `b`, `[F(x)]` y `a`.
- La secuencia visible es obligatoria: integral definida completa; primitiva evaluada; sustitución del límite superior; resta del valor del límite inferior; simplificación y resultado.
- La regresión debe inspeccionar la geometría real del DOM, no solo la presencia del string fuente. A 1280, 768, 375 y 320 px debe comprobar que ambos límites son legibles, permanecen ligados al operador correspondiente y no producen desbordamiento horizontal general.

Caso de regresión conocido de Andalucía, Ordinaria 2017: para las curvas `y=x²` y `y=−x²+4x`, calcular los cortes `(0,0)` y `(2,4)`, identificar `−x²+4x` como techo y `x²` como suelo en `[0,2]`, sombrear exclusivamente la franja entre ambas y plantear `A=∫_0^2[(−x²+4x)−x²]dx`. Debe existir una sola gráfica, con las dos ecuaciones rotuladas y ambos puntos de corte señalados.

Caso de regresión conocido de Andalucía, Extraordinaria 2020: para `f(x)=|x|` y `g(x)=x²−2`, representar una V y una parábola, marcar e identificar los dos cortes `(-2,2)` y `(2,2)`, rotular las curvas con sus expresiones reales y sombrear exclusivamente el recinto matemático calculado. No aceptar leyendas genéricas como «techo», «suelo», «curva 1» o textos de la solución en lugar de las ecuaciones; debe existir una sola gráfica salvo que una segunda aporte información matemática distinta.
...
5. Calcular la inversa mediante determinantes usando la nomenclatura del centro: construir directamente `Adj(B)` como una matriz cuyos elementos son `(-1)^(i+j)·det(Mᵢⱼ)`. No crear ni nombrar una fase separada de cofactores, no denominarla `Cof(B)` y no usar `Adj(B)` para una matriz ya traspuesta. Mostrar sin saltos `det(B)`, la matriz simbólica `Adj(B)` con cada determinante menor `2×2` escrito en su posición, la matriz numérica `Adj(B)`, su traspuesta `Adj(B)^T` y `B⁻¹=(1/det(B))·Adj(B)^T`. Para matrices de orden dos puede aplicarse la fórmula equivalente mostrando también el determinante.
...
Para ecuaciones de parámetros del tipo `(A+aI)²=bI`, el desarrollo principal debe ser matricial y visible: escribir `A` e `I` con su orden; construir y mostrar `A+aI`; escribir `(A+aI)(A+aI)`; efectuar el producto fila por columna; mostrar la matriz resultado; escribir `bI`; igualar entradas; resolver las ecuaciones escalares y comprobar los parámetros. No sustituir este procedimiento por un atajo de traza y determinante.

Para `MX+M²=I`, escribir `M`, calcular y mostrar `M²`, sustituir en la ecuación, pasar a `MX=I−M²`, calcular la matriz del segundo miembro, despejar `X` por el lado correcto mediante la inversa cuando exista, mostrar `X` y comprobarla en la ecuación original.
...
Los materiales originales de CCSS I incluyen una tabla acumulada de la normal típica cuyos valores representan `P(Z≤z)`.
...
- para `z≥0`, buscar directamente `P(Z≤z)`;
- para valores negativos, usar `P(Z≤−z)=1−P(Z≤z)`;
- para colas derechas, usar `P(Z>z)=1−P(Z≤z)`;
- para intervalos, restar las probabilidades acumuladas correspondientes.

Mostrar la transformación realizada, indicar literalmente qué probabilidad acumulada se busca, escribir el valor leído y sustituirlo. No usar `Φ`, `Φ⁻¹`, `invNorm` ni otra abreviatura como intermediario pedagógico, salvo que el propio enunciado oficial la contenga y deba conservarse literalmente.
...
- Heredar de CCSS I, cuando corresponda: probabilidad condicionada, independencia, probabilidad total, Bayes, binomial exacta, probabilidades acumuladas, normal ordinaria y la tabla acumulada a la izquierda expresada como `P(Z≤z)`.
...
- Presentar toda colección de ecuaciones simultáneas como un único sistema: una sola llave común, una fila alineada por ecuación y el mismo número de filas en la fuente, el canónico, el runtime y el DOM. Esta regla se aplica también a los sistemas auxiliares que aparecen al calcular intersecciones, combinaciones lineales o vértices. No convertir en sistema una mera sucesión narrativa de igualdades independientes.
...
- La evaluación de la función objetivo debe aparecer como tabla explícita `vértice | valor`, con todos los vértices factibles. El cierre debe interpretar el óptimo en el contexto, incluir unidades, sobrantes u holguras pedidos y distinguir un punto óptimo único de un segmento completo de óptimos.
- Clasificar en Programación lineal únicamente cuando el núcleo del enunciado sea modelizar u optimizar sobre una región factible. La aparición incidental de un sistema, una inecuación o un determinante no cambia el tema principal; a la inversa, los sistemas usados para hallar vértices son herramientas secundarias y no desplazan el ejercicio a Sistemas.
...
- En CCSS II la tabla normal representa el acumulado a la izquierda `P(Z≤z)`. Para una cola derecha usar `1−P(Z≤z)`; para un intervalo, la diferencia de probabilidades acumuladas; para valores negativos, la simetría correspondiente. Indicar qué valor se busca en la tabla, escribir el valor leído y sustituirlo, sin usar `Φ` como intermediario.
...
- El procedimiento completo debe mostrar, en este orden: identificación de `n` y `p`; cálculo de `q`; comprobación de `np≥5` y `nq≥5`; cálculo de `μ` y `σ`; escritura de la normal aproximante; escritura del suceso binomial original; corrección de continuidad; tipificación `Z=(Y−μ)/σ` dentro de la probabilidad; transformación a una probabilidad acumulada `P(Z≤z)` consultable; lectura explícita de la tabla; cálculo; comprobación `0≤P≤1`; e interpretación contextual cuando corresponda.
...
### Regresiones obligatorias para bancos PAU interactivos

- La unidad de reconciliación es `PDF → ejercicio → apartados → canónico → runtime → DOM`. Para cada ejercicio habilitado, el número y las etiquetas de apartados originales deben coincidir exactamente con los canónicos y con los interactivos. No dividir una pregunta sin apartados en preguntas artificiales ni fusionar `b)`, `c)`, `d)` o `e)`; un caso no convertible queda bloqueado con expediente verificable.
- En Matemáticas II, toda expansión de un determinante de orden 3 debe escribir antes de simplificar cada `elemento × determinante del menor complementario 2×2` real. Después se calculan los menores y se simplifica. No basta con saltar directamente al polinomio resultante.
- Cuando se busque un punto de una recta espacial, escribir primero la parametrización, después la frase exacta «Sea C un punto genérico de la recta.» y a continuación las coordenadas paramétricas de `C`.
- En límites de 2.º de Bachillerato Matemáticas II, L’Hôpital solo puede aparecer después de escribir explícitamente `0/0` o `∞/∞`. Si al derivar persiste la indeterminación, volver a nombrarla antes de repetir la regla. No usar Taylor, Maclaurin ni términos `O(·)` como método por defecto. Mantener en 1.º de Bachillerato la prohibición específica de L’Hôpital.
- Para crecimiento, decrecimiento, máximos y mínimos, incluir una recta real con los ceros y signos de `f′`; para concavidad, convexidad e inflexión, la recta de signos de `f′′`. El mero criterio de la segunda derivada no sustituye estas rectas cuando el usuario las exige.
- Toda área entre curvas o respecto del eje debe incorporar un dibujo cartesiano con ejes, funciones, intersecciones, región sombreada, techo/suelo o signo por intervalos, y terminar con área positiva. Si la gráfica no puede construirse con seguridad, el ejercicio no se habilita.
- En distribución normal o binomial se conservan todos los apartados y se declara el recurso exacto que necesita el alumno. La tabla normal debe poder abrirse desde el ejercicio cuando sea necesaria. No reconstruir como binomial un modelo que la fuente no identifica como tal.
- La entrega falla si el DOM visible contiene `matrix`, `matrix{`, `matrix(`, `system{`, `frac{}`, delimitadores TeX o cualquier token interno. Probar enunciado, opciones, corrección y solución, tanto en escritorio como en móvil.
...
### Regresión común para bancos oficiales interactivos

- Distinguir apartado oficial de subdivisión pedagógica. Se permite descomponer un apartado en ítems interactivos (`a.i`, `a.ii`, funciones `f` y `g`, etc.) únicamente si se conserva íntegro el literal común, el orden documental y cada pregunta exigida; registrar la relación padre–hijo. Nunca fusionar preguntas independientes.
- Si el examen contiene `Parte I` y `Parte II`, conservar ese orden incluso cuando el solucionario o la importación lleguen invertidos. El identificador visible y el historial deben mantener la jerarquía (`I.a`, `I.b`, `II.a`...).
- En CCSS II no utilizar L’Hôpital. Resolver límites por factorización, racionalización, división por la potencia dominante, límites fundamentales u otra técnica del temario. La frase «sin usar L’Hôpital» no constituye una infracción en las auditorías.
- En distribución normal escribir primero la probabilidad pedida, tipificar después y usar `P(Z≤z)` o la desigualdad equivalente; no presentar `Φ(z)` como sustituto opaco de ese paso. En tamaños muestrales, comprobar el entero mínimo y redondear siempre hacia arriba.
- En cualquier lectura directa o inversa de la normal, mostrar la cadena completa: probabilidad original, tipificación, probabilidad en `Z`, transformación a un acumulado consultable, frase «Buscamos en la tabla…», valor leído, sustitución, operación e interpretación. En el cálculo inverso, buscar el valor de `z` cuya probabilidad acumulada es la dada y después despejar en `z=(x−μ)/σ`; no usar `Φ⁻¹` ni `invNorm` como sustitución de la lectura pedagógica.
- Las cantidades con unidad forman un bloque indivisible (`64 kg`, `1,68 m`, `25 kg/m²`, `10 km²`). Mantener un espacio no separable y comprobar que valor y unidad no saltan a líneas distintas.
- Andalucía y Madrid muestran solo el recurso estadístico exacto mediante un desplegable que no reinicia la respuesta. Castilla-La Mancha conserva la tabla local del propio ejercicio. Una tabla auxiliar independiente nunca se censará como examen.
- Para áreas, el dibujo debe corresponder al desarrollo: ejes, puntos de corte, techo/suelo o signo por intervalos y región sombreada. Un esquema cualitativo debe declararse como tal y remitir a los valores exactos calculados; no puede sustituir una gráfica fuente imprescindible.
- La comprobación automática debe cubrir enunciado, cuatro opciones distintas, corrección, solución, tabla auxiliar y gráfico a 375, 768 y 1280 px. Falla si aparece sintaxis interna, desbordamiento horizontal de página o pérdida de estado al abrir/cerrar recursos.
- La salida visible falla si conserva decimales técnicos como `1{,}8`, comandos como `\\approx` o cualquier comando TeX interno en enunciado, apartado, opciones, corrección, solución o respuesta final. La barrera exigida es `VISIBLE_INTERNAL_MATH_TOKENS=0` en los tres anchos de control.
- La regresión metodológica de normal debe comprobar que la tipificación aparece dentro de la probabilidad, que se nombran los valores acumulados buscados y leídos y que no aparecen `Φ`, `Φ⁻¹` ni `invNorm`.
- La regresión de derivadas de cocientes debe rechazar las abreviaturas artificiales `N`, `D`, `N'`, `D'` cuando la regla cabe directamente. La regresión de asíntotas debe exigir límites laterales en verticales, límites a ambos infinitos en horizontales y los límites de `m` y `n` en oblicuas.
- La regresión de áreas debe comprobar cortes, ecuaciones rotuladas, techo/suelo o signo, integral completa previa a Barrow, una única gráfica y sombreado limitado exactamente por las fronteras calculadas.
